...
public class UpdateCommentServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String comment = jsonData.getString("changedDescription");
		CommentService updateComment = new CommentService();
		try {
			if(updateComment.editCommentService(commendId, comment)) {
				out.println("Success");
			}else {
				out.println("Failed");
			}
